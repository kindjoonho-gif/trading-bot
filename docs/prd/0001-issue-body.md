<!--
After `git init` + `gh repo create`, publish this PRD as a GitHub issue:

  gh issue create `
    --title "Phase A: KIS (KOSPI) manual orders and Portfolio Rebalance" `
    --body-file docs/prd/0001-issue-body.md `
    --label ready-for-agent

The body below is identical to docs/prd/0001-phase-a-kis-rebalance.md but with
the leading status block stripped so the title carries it. Keep the two files
in sync if the PRD changes before the issue is filed.
-->
...
- **Adapter pattern over a `Broker` Protocol.** One concrete adapter per Venue. Phase A ships `KISBroker`. The Protocol surface is intentionally small; Venue-specific extras live on the concrete class, not the Protocol.
...
- `trader.brokers.base` — `Broker` Protocol. Async. Methods: `get_cash`, `get_positions`, `get_quote`, `place_order`, `get_order`, `list_open_orders`, `cancel_order`. History/P&L endpoints to be added as Protocol methods after KIS endpoint inventory is finalized; if Venue-specific they stay on the concrete adapter.
...
- `trader.domain.types` — `Symbol`, `OrderId` NewTypes; `Side`, `OrderKind`, `OrderStatus` enums; `Position`, `Quote`, `Order` pydantic models (frozen, `extra=forbid`).
- `trader.domain.money` — `to_decimal`, `round_toward_zero`, `format_krw`.
...
- `trader.rebalance.plan` — **deep pure module.** Signature: `(positions, cash, target_weights, quotes, tolerance) → Plan`. Returns a list of `(Symbol, Side, kind=MARKET, quantity)` entries plus residual diagnostics (Total Value, per-Symbol current weight, drift, raw Δ shares, rounded Δ shares, skipped-for-tolerance flag). No I/O.
...
### Key behavioral decisions (from the grilling session)

- **Order types:** Market and Limit only. Stop-Loss explicitly Phase B because true server-side stops are limited on KOSPI and client-side stop monitoring brings forward Phase B's monitor-loop infrastructure.
- **Quote source:** REST snapshot only. WebSocket tape deferred to Phase B.
- **Rebalance Order type:** all Market. Limit-at-touch introduces fill-watch/retry complexity that crosses into Phase B.
- **Rebalance compute model:** Target Weights are fractions of Total Value (`cash + Σ positions × price`). Sum must be `≤ 1.0`; residual is Cash, implicit. Per-Symbol Δ shares rounded toward zero so no Order overspends cash or oversells. Drift Tolerance (default 1%) gates whether a Symbol is included in the Plan.
- **Symbol convention:** bare 6-digit string. The Portfolio file is bound to one Broker, so the Venue is unambiguous. Promoting to a `Symbol(market, code)` type is a trivial future migration when mixed-Venue baskets land.
...
- **No own database.** Positions, open Orders, Fill history, realized P&L all come from KIS endpoints. History is bounded by KIS retention (months, not years). A SQLite-backed history store is explicit Phase B.
...
- `trader.rebalance.plan` — exact-fit allocation, residual cash, Symbol below Drift Tolerance skipped, sell side rounded toward zero, buy side rounded toward zero, single-Symbol Portfolio, all-cash Portfolio, sum-of-weights == 1.0, target weight = 0 for an existing Position (full liquidation), price unchanged from a prior compute is idempotent.
...
- `trader.tickers.master` — network fetch + CSV write. Smoke test only.

### Prior art

This is a new repo; there is no prior test corpus. Layout: `tests/unit/` for pure-module tests, `tests/integration/` for KIS Mock Account tests guarded by a `KIS_INTEGRATION=1` env gate so they do not run by default in CI without credentials.
...
- US equities and cryptocurrency Venues. The `Broker` Protocol accommodates them; adapters are not built here.
- Mixed-Venue Portfolios. One Portfolio file = one Broker = one Venue.
- Own persistent database. Fill history and realized P&L come from KIS endpoints only.
- Order modification ("amend") and replace-after-cancel chaining. Place and cancel only.
- TWAP, VWAP, iceberg, post-only, IOC, FOK or any non-Market/Limit order type.
- Pre-market / after-hours sessions.
- Per-asset Limit override in Rebalance.
- Mobile or non-localhost web deploy. Streamlit runs locally only.
- Multi-user / authentication.

## Further Notes

- **KIS API specifics to handle in `brokers.kis`:** access token has a 24-hour TTL (cache to disk under `.cache/`); `approval_key` for WebSocket is a separate flow (not needed here); rate limit ≈ 20 req/s on real, ≈ 2 req/s on mock (verify per endpoint at implementation time); `CANO` and `ACNT_PRDT_CD` are split from the configured account number.
